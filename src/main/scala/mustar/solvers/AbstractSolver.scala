...
import libra.graphs.ConstantGraph.{ConstantGraph, EdgeLabel}
import libra.automata.DFA
import libra.graphs.CTBigraph.CTBigraph
import libra.graphs.{CTBigraph, ConstantGraph, Edge}
import libra.nonrecursiveQueries.{Rule, SPJ, UCQ}
import libra.util.{Logging, Timed}

abstract class AbstractSolver(val problem: Problem) {

  val Problem(problemName, inputRelations, outputRelations, types, hypothesisSpace) = problem
